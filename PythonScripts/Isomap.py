...
isomap_results = isomap.fit_transform(processed_vectors)

for keyword in set(all_keywords):
    keyword_indices = [i for i, k in enumerate(all_keywords) if k == keyword]
    keyword_results = [(original_indices[i], isomap_results[i]) for i in keyword_indices]
    keyword_results.sort(key=lambda x: x[0])

    output_file = os.path.join(outputfile_directory, f'{keyword}_isomap_nn{n_neighbors}_results.txt')
